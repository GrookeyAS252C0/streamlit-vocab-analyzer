...
class ReadingAssistApp:
    ...
    def load_config(self):
        """設定ファイルの読み込み"""
        config_path = current_dir / "reading_assist_analyzer" / "config" / "settings.json"
        try:
    ...
    def handle_sample_text(self):
        ...
        if st.button("サンプル読み込み"):
            sample_path = current_dir / "reading_assist_analyzer" / sample_files[selected_sample]
        ...
        with st.spinner("分析を実行中..."):
            try:
                ...
                result = self.text_analyzer.analyze_text_comprehensive(
                    st.session_state.input_text,
                    str(current_dir / "reading_assist_analyzer" / "data" / "vocabulary_books")
                )
